...
function G.UIDEF.use_and_sell_buttons(card)
    ...
    if card.area == G.pack_cards and G.pack_cards and ((card.ability.set == "RCode" or card.ability.set == "CBlind" or card.ability.set == "RTarot" or card.ability.set == "RSpectral") or not SMODS.OPENED_BOOSTER.draw_hand and card.children.front) and (card.ability.consumeable) then
        return {
            n = G.UIT.ROOT,
...
function create_UIBox_blind_select()
    ...
    if G.GAME.round_resets.red_room then
        ...
        if Entropy.CanEeSpawn() then
            if G.GAME.EEBuildup or G.GAME.round_resets.ante_disp == "32" then
...
function Game:update(dt)
	...
	if cdt > 0.01 then
		...
		if G.jokers and G.GAME.blind and G.GAME.blind.config.blind.key == "bl_entr_endless_entropy_phase_three" then
            ...
            for i = 1, math.floor(math.max(G.jokers.config.card_limit/5, 1))
