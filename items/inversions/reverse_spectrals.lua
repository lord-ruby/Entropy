...
local quasar = {
    ...
    use = function(self, card, area, copier,amt)
        ...
        G.E_MANAGER:add_event(Event({
          ...
          func = function()
            play_sound("tarot1")
            ease_colour(G.C.UI_CHIPS, copy_table(Entropy.get_asc_colour(to_big(G.GAME.hands[i].level) * to_big(amt) * to_big(0.5))), 0.1)
            ease_colour(G.C.UI_MULT, copy_table(Entropy.get_asc_colour(to_big(G.GAME.hands[i].level) * to_big(amt) * to_big(0.5))), 0.1)
