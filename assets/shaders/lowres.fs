...
extern MY_HIGHP_OR_MEDIUMP vec2 lowres;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float cx = uv_scaled_centered.x * 1.5;
    float cy = uv_scaled_centered.y * 1.5;
    ...
    float res = (.5 + .5* cos( (lowres.x) * 2.612 + ( field + -.5 ) *3.14));
    vec4 textp = RGB(hsl);
    tex.rgb = textp.rgb;
